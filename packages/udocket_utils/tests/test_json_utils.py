...
import pytest
from udocket_utils import json_utils as ju
...
def test_normalize_mapping_identity() -> None:
    result = cast("dict[str, int | str]", ju.normalize_mapping({"A": 1, 2: "two"}))
    assert result == {"A": 1, "2": "two"}


def test_normalize_mapping_with_transform() -> None:
    result = ju.normalize_mapping({"A": 1, "B": 2}, transform=_times_ten)
...
def test_normalize_mapping_optional_with_transform() -> None:
    result: dict[str, int] = ju.normalize_mapping_optional({"a": 1, "b": 2}, transform=_double)
...
def test_coerce_bool_variants(value: object, expectation: str) -> None:
    expectation_map: dict[str, bool | None] = {"true": True, "false": False, "none": None}
    assert ju.coerce_bool(value) == expectation_map[expectation]
